...
async function rsend(localDirectory, remoteDestination, {shasumFile, clean}={shasumFile:'SHA256SUM', clean:false}){
  ...
  const localShasum = path.join(localDirectory, shasumFile);
  ...
  if(checksumMissing){
    log(`Checksum file missing in ${remoteDirectory} creating a new blank one.`);
    await exec("ssh", [ remoteHost, `sha256sum ${path.join(remoteDirectory, '*.*' )} || echo "" > ${path.join(remoteDirectory, shasumFile )}` ]);
  }

  const previousChecksums = (await exec("ssh",[remoteHost, `cat ${path.join(remoteDirectory, shasumFile)}`])).stdout.trim().split("\n").filter(i=>i).map(l=>l.split('  '));
  const currentChecksums = (await readFile(localShasum)).toString().split("\n").map(l=>l.split('  '));
  log(`Number of files in current list: ${currentChecksums.length}`);
  ...
  log(`Files shared between previous list and current list: ${duplicateChecksums.length}`);
  ...
  if(filesNeedDeletion){
    if(clean){
      ...
      log(`Discovered ${expiredFiles.length} file${expiredFiles.length==1?'':'s'} that require${expiredFiles.length==1?'s':''} deletion use --clean to remove ${expiredFiles.length==1?'it':'them'}.`);
      console.log(expiredFiles);
    }
  }

  let existingFiles = (await exec("ssh", [ remoteHost, `ls -1 ${remoteDirectory}/*.* || echo`, ])).stdout.split('\n').map(i=>i.trim()).filter(i=>i).map(i=>path.basename(i));
